...
function Feed({user,profileInfo,allFeedPosts}) {
  ...
  function handleFileOnChange(event) {
   ...
   function handleFetchImagePublicUrl(getData){
    const {data} =  supabaseClient.storage.from('work-wave-public').getPublicUrl(getData.path)
    console.log(data);
  ...
  return (
    <Fragment>
      <div className="mx-auto max-w-7xl">
        <div className="flex items-baseline justify-between dark:border-white border-b pb-6 pt-24 border-cyan-500">
          <h1 className="dark:text-white text-4xl font-bold tracking-tight text-cyan-900">
            Explore Your Feed...
          </h1>
      ...
      >
        <DialogContent className="h-[550px]">
          <Textarea
            ...
            className="border-none outline-none focus-visible:ring-0 focus-visible:ring-offset-0 h-[200px] text-[26px]"
          />
